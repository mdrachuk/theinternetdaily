...
runs only when a server is reachable::
    ...
    PAPERNEWS_TEST_MONGO_URI=mongodb://localhost:27017 uv run pytest
...
def _mongo_uri() -> str | None:
    return os.environ.get("PAPERNEWS_TEST_MONGO_URI")
...
async def store(request, tmp_path):
    ...
    uri = _mongo_uri()
    if not uri:
        pytest.skip("set PAPERNEWS_TEST_MONGO_URI to exercise the Mongo store")
    ...
    s = MongoStore(uri=uri, database="papernews_test", collection=collection)
    try:
        yield s
    finally:
        await s.col.drop()
